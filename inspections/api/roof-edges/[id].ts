...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (req.method === 'PATCH') {
    const { label, lengthM, atikaHeightCm } = req.body || {}
    ...
    if (atikaHeightCm !== undefined) {
      data.atikaHeightCm = atikaHeightCm === '' || atikaHeightCm === null ? null : Number(atikaHeightCm)
    }

    try {
      const edge = await prisma.roofEdge.update({ where: { id }, data })
  ...
  if (req.method === 'DELETE') {
    try {
      await prisma.roofEdge.delete({ where: { id } })
